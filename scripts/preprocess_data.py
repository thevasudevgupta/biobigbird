...
data = load_dataset("parquet", data_files='train-00000-of-00389-a3285e04b5e3defa.parquet', split="train")
print(data)
...
def preprocess(text):
    ...
    new_paragraphs = [paragraph.strip() for paragraph in paragraphs[start_idx:end_idx+1]]
    new_paragraphs = [paragraph for paragraph in new_paragraphs if paragraph != '' or not paragraph.startswith('fig.')]
...
data = data.map(lambda x: {"preprocessed": preprocess(x['article'])}, load_from_cache_file=False)
print(data)
